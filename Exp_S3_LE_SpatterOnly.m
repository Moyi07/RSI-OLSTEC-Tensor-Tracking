%% Experiment S3 LE Spatter-Only
% =========================================================================
% Prerequisites: Run `run_me_first.m` once to add all dependencies to path.
%
% Description:
%   This script is a spatter-only ablation of the event-driven S3 benchmark.
%   It removes Gaussian observation noise N_t while preserving the smooth
%   dynamics, event-driven subspace changes, sparse impulsive corruption,
%   sampling ratio, auxiliary signal, and algorithm settings.
% =========================================================================
clear; clc; close all;

%% 1. Global Configuration
% -------------------------------------------------------------------------
fprintf('Starting Synthetic Experiment S3 LE Spatter-Only...\n');
...
SNR_dB          = Inf;          % Gaussian observation noise removed for this ablation
tolcost         = 1e-8;
maxepochs       = 1;
verbose         = 0;
n_monte_carlo   = 50;           % Number of Monte Carlo trials.
export_results  = true;
result_dir      = fullfile(fileparts(mfilename('fullpath')), 'result', 'S3_LE_SpatterOnly');
...
function run_s3_monte_carlo(tensor_dims, rank_r, fraction, sparse_ratio, ...
    ...
    spatter_seed_list = 300000 + seed_list;
    ...
    for trial = 1:n_monte_carlo
        ...
        rng(spatter_seed_list(trial), 'twister');
        S_mask = rand(rows, cols, total_slices) < sparse_ratio;
    ...
    if any(algorithm_status(:) == 0)
        error('Exp_S3_LE:IncompleteExecutionState', ...
    ...
    title(sprintf('S3 LE Spatter-Only Benchmark with Smooth Dynamics and Event Changes (n=%d)', n_monte_carlo), ...
    ...
    fprintf('Observation model: L_t + E_t (Gaussian observation noise disabled).\n');
    ...
    if export_results
        ...
        save(fullfile(result_dir, 'S3_LE_SpatterOnly_stats.mat'), ...
            'matrix_rank', 'tensor_dims', 'rank_r', 'fraction', 'sparse_ratio', 'SNR_dB', ...
            'noise_sigma', 'tolcost', 'maxepochs', 'n_monte_carlo', 'seed_list', ...
            'clean_seed_list', 'auxiliary_seed_list', 'spatter_seed_list', ...
        ...
        writetable(failure_log, fullfile(result_dir, 'S3_LE_SpatterOnly_failure_log.csv'));
        writetable(failure_summary, fullfile(result_dir, 'S3_LE_SpatterOnly_failure_summary.csv'));
        savefig(fig, fullfile(result_dir, 'S3_LE_SpatterOnly.fig'));
        set(fig, 'Units', 'Inches');
        pos = get(fig, 'Position');
        set(fig, 'PaperPositionMode', 'auto', 'PaperUnits', 'Inches', 'PaperSize', [pos(3), pos(4)]);
        print(fig, fullfile(result_dir, 'S3_LE_SpatterOnly.pdf'), '-dpdf', '-vector');
        print(fig, fullfile(result_dir, 'S3_LE_SpatterOnly.eps'), '-depsc', '-vector');
    end
