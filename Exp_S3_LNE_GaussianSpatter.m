%% Experiment S3 LNE Gaussian-Spatter
% =========================================================================
% Prerequisites: Run `run_me_first.m` once to add all dependencies to path.
%
% Objective:
%   Evaluate a single synthetic benchmark setting.
%   It combines event-driven subspace changes with sparse impulsive noise,
%   so full RSI-OLSTEC uses both adaptive lambda and Huber robustness while
%   being compared against standard matrix/tensor baselines.
% =========================================================================
clear; clc; close all;

%% 1. Global Configuration
% -------------------------------------------------------------------------
fprintf('Starting Synthetic Experiment S3 LNE Gaussian-Spatter...\n');
...
export_results  = true;
result_dir      = fullfile(fileparts(mfilename('fullpath')), 'result', 'S3_LNE_GaussianSpatter');
...
function run_s3_monte_carlo(tensor_dims, rank_r, fraction, sparse_ratio, ...
    ...
    aux_step_gain = 10.0;
    aux_event_decay = 20;
    condition_name = 'Gaussian-spatter';
    ...
    if any(algorithm_status(:) == 0)
        error('Exp_S3_LNE:IncompleteExecutionState', ...
    ...
    title(sprintf('S3 LNE Gaussian-Spatter Benchmark with Smooth Dynamics and Event Changes (n=%d)', n_monte_carlo), ...
    ...
    if export_results
        ...
        save(fullfile(result_dir, 'S3_LNE_GaussianSpatter_stats.mat'), ...
            ...
            'gaussian_seed_list', 'mask_seed_list', 'initialization_seed_list', ...
        ...
        writetable(failure_log, fullfile(result_dir, 'S3_LNE_GaussianSpatter_failure_log.csv'));
        writetable(failure_summary, fullfile(result_dir, 'S3_LNE_GaussianSpatter_failure_summary.csv'));
        savefig(fig, fullfile(result_dir, 'S3_LNE_GaussianSpatter.fig'));
        set(fig, 'Units', 'Inches');
        pos = get(fig, 'Position');
        set(fig, 'PaperPositionMode', 'auto', 'PaperUnits', 'Inches', 'PaperSize', [pos(3), pos(4)]);
        print(fig, fullfile(result_dir, 'S3_LNE_GaussianSpatter.pdf'), '-dpdf', '-vector');
        print(fig, fullfile(result_dir, 'S3_LNE_GaussianSpatter.eps'), '-depsc', '-vector');
    end
