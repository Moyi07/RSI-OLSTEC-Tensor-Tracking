...
% Created by H.Kasai on June 10, 2017
    ...
    % Check image size.
    if m ~= rows * cols
        error('load_realdata_tensor:ImageSizeMismatch', ...
            'Loaded image size (%d) is different from user definition (%d x %d = %d).', ...
    ...
    max_abs_signal = max(abs(Matrix_Y_Noiseless(:)));
    if max_abs_signal > 0
    ...
    % Normalize tensors and matrices.
